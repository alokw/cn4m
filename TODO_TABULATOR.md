...
- [x] **QC filter** *(done — needs testing)*: rows are stamped with `qc_fail` during the transform; a **QC FAILS ONLY** toggle sits next to the audio buttons.
  ...
  - **Follow-up: SELECT ALL FLAGGED is a toggle** — flips to `UN-SELECT ALL FLAGGED` once every visible flagged row is selected. The direction is *derived* from the current selection rather than a stored boolean, so manually unticking one flagged row flips the label back to "select" instead of leaving the button out of step with the table.
  - **Follow-up: right-click a cell to filter by it.** A `contextMenu` on `columnDefaults` offers `Filter by "wav"`, plus `Clear this column's filter` (only when one is set) and `Clear all filters`. It writes into the column's own header filter, so the filter stays visible and clearable the normal way rather than becoming hidden state.
    - Duration and Size need conversion — they *display* a timecode and `1.2 GiB` but *filter* in seconds and MiB. The menu converts from the raw field and rounds to 3dp, which stays inside `operator_match`'s 0.001 tolerance, so the clicked row always matches itself (tested against an awkward 686.7397 MiB value).
